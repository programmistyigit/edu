...
router.put("/businessmen/image", middlewareBusinesmen, async (req, res) => {
    const id = req.id;
    const findBusiness = await BusinesMenSChema.findOne(id);

    if (!req.files || Object.keys(req.files).length === 0) {
        return res.status(400).send("No files were uploaded.");
    }

    const uploadedFile = req.files.business_avatar; // "uploadedFile" is the name of the file input field in your HTML form
    ...
    uploadedFile.mv(uploadPath, (err) => {
        ...
        sharp(uploadPath)
            ...
            .toFile(uploadPath, (sharpErr, info) => {
                ...
                const imageUrl = `${host}/images/${uniqueFilename}`;
        findBusiness.mother_avatar = imageUrl;
                findBusiness.save()
                    .then(() => {
                        res.json({ message: "File uploaded with unique filename: " + uniqueFilename });
                    })
...
router.put("/businesmen/library/files/:id", middlewareBusinesmen , async (req, res) => {
    const useOwn = req.id
    const libraryId = req.params.id

    if(!mongoose.Types.ObjectId.isValid(libraryId)) return res.status(400).json( { status : "error" , message : "invalid class id !"})
    
    const findLibrary = await libraryesSchema.findById(libraryId)

    if(!findLibrary) return res.status(400).json({ status : "warning" , message : "Library not found"})
    if(findLibrary.library_businesmen.toString() != useOwn) return res.status(400).json( { status :"warning" , message : "forbidden attempt"})

    if (!req.files || Object.keys(req.files).length === 0) {
        return res.status(400).send("No files were uploaded.");
    }
    const uploadedFile = req.files.library_avatar;
    if(!uploadedFile) return res.status(400).json( { status : "error" , message : "file not found"})
    const uniqueFilename = uuid() + "." + uploadedFile.name.split(".").pop(); // Adds UUID to the original file extension
    uploadedFile.mv("uploads/" + uniqueFilename)
    sharp(uploadedFile.data)
        ...
        const imageUrl = `${host}/images/${uniqueFilename}`;
        findLibrary.library_avatar = imageUrl
        await findLibrary.save()
        res.status(200).json( { status: "success" , message : "avatar uploaded" , url : imageUrl })
})
